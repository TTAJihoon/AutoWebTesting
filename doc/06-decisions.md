...
| **D54** | **Leaf 그룹핑 최적화** — TC_DESIGN을 leaf 1개가 아닌 **source_url(페이지) 그룹** 단위로 호출(과대 페이지는 cap12 분할). LLM이 같은 화면 기능 관계를 보고 cross-feature TC·중복제거. 실측: 2,518 leaf→89 페이지그룹→247 호출(90%↓), 관계 응집 최상. leaf당 happy_path 보장(손실 0). 상세: [doc/09-tc-grouping-and-performance.md](09-tc-grouping-and-performance.md) §2 | 2026-06-02 |
