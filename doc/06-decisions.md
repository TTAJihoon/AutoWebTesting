...
| **D51** | **전역 컴포넌트 dedup** — 헤더·푸터·네비 등 ≥`GLOBAL_RATIO`(0.5) 페이지에 동일 셀렉터 지문으로 반복되는 요소를 `__global__`로 1회만 명세. 규칙 기반(LLM 불필요), 손실 0(이동), `dedup_global_components` 옵트아웃. 로그인 편중(인증 도메인 ~30%)의 1순위 원인 직격. 상세: [doc/08-feature-list-refinement.md](08-feature-list-refinement.md) §3 | 2026-06-02 |
