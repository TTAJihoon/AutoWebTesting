...
| **D54** | **Leaf 그룹핑 최적화** — (A) TC_DESIGN을 leaf 1개가 아닌 **source_url(페이지) 그룹** 단위로 호출(cap12 분할). LLM이 같은 화면 기능 관계를 보고 cross-feature·중복제거. 실측 2,518→89그룹→247호출(90%↓). leaf당 happy_path 보장(손실 0). (B) **교차 페이지 플로우 패스(TC_FLOW)** — 사이트맵 요약 입력, 로그인→글쓰기→목록 등 end-to-end 시나리오 TC를 소수 호출로 설계. **1차 포함 확정**. cap=12·동시성=6·Qt signal 마샬링 확정(2026-06-02 검토). 상세: [doc/09-tc-grouping-and-performance.md](09-tc-grouping-and-performance.md) §2 | 2026-06-02 |
| **D55** | **LLM 호출 병렬화** — 독립 호출(그룹·DOM 청크·V10 gap·실패분석)을 `ThreadPoolExecutor(기본 6)` 동시 실행. 스레드 안전(`_last_call_time` Lock·결과 입력순 재정렬로 결정성 유지). `CONCURRENCY=1`이면 순차(옵트아웃). 상세: [doc/09-tc-grouping-and-performance.md](09-tc-grouping-and-performance.md) §2 | 2026-06-02 |
| **D56** | **V10 보완 재설계** — 현행 gap마다 TC_DESIGN 순차(777회·TC 10배 증식)를 ① D54 그룹 설계에 negative 카테고리 선제 포함→gap 감소 ② 남은 gap 페이지 배치 ③ 병렬화 ④ 증식 상한으로 대체. D49 커버리지 기준은 유지. TC 생성 단일 최대 병목 제거. 상세: [doc/09-tc-grouping-and-performance.md](09-tc-grouping-and-performance.md) §2 | 2026-06-02 |
| **D57** | **Reviewer Gate 리스크 triage** — TC를 `gen_confidence`+`source_quote`(MANUAL/INVARIANT/INFERRED) 규칙으로 🔴집중(INFERRED&conf<0.45, 실측22%)/🟢안전(근거확실&conf≥0.7, 23%)/🟡확인(55%) 3버킷 자동 분류. 🟢 일괄승인+🔴 집중+위험군 기준 진행률 → "꼭 정독" 100%→~22%로 검토 부담↓. 필터·정렬·"왜 검토하나" 배지. 임계값 노출. 상세: [doc/10-reviewer-gate-v2.md](10-reviewer-gate-v2.md) §3 | 2026-06-02 |
