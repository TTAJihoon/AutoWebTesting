...
> 최종 갱신: 2026-05-20 | 대상 OS: Windows 11 Pro 23H2
...
1. 로그인 (DB 계정)
2. 대시보드 → "새 실행" → 마법사
3. 매뉴얼 파일 + 대상 URL + 인증 입력
4. Stage 진행 표시 (Pipeline View)
5. Stage 3 완료 시 **Reviewer Gate** 자동 표시 (TC당 30초~2분 — D20)
6. 결정 입력 → Stage 5 자동 실행
7. 완료 시 대시보드 이력에서 `tc_final.xlsx` 열기
...
### 7.2 색상 코드 (gen_confidence 기반)
| 색 | confidence | 의미 | 권장 시간 |
|---|---|---|---|
| 회색 | ≥ 0.85 | 명확한 매뉴얼 근거 | 5초 — quick approve |
| 노랑 | 0.50 ~ 0.85 | 매뉴얼 일부 모호 | 5~30초 — 확인 |
| 빨강 | < 0.50 또는 INFERRED | 추론 의존 | 30초~2분 — 심층 |
